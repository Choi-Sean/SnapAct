...
mobile/    Expo(React Native) 앱 (사진 선택/촬영 → 업로드 → 결과 표시 → 연락처/캘린더 저장)
```
...
4. 배포 후 발급되는 URL(`https://xxx.up.railway.app`)을 `mobile/src/config.ts`의 `API_BASE_URL`에 넣으세요.
...
- 로컬 백엔드로 테스트하려면 `mobile/src/config.ts`의 `API_BASE_URL`을 **PC의 LAN IP**로 맞춰주세요 (휴대폰과 PC가 같은 Wi-Fi여야 함). 현재 PC IP 기준으로 `http://10.0.0.244:8000` 이 기본값으로 들어가 있습니다. IP가 바뀌면 `ipconfig`(Windows)로 다시 확인해서 수정하세요.
...
## 4. 이번 데모에서 빠진 것 (다음 단계)

- **Share Extension(iOS) / Share Intent(Android)**: Photos 앱 공유 버튼에서 바로 실행되는 기능은 네이티브 빌드(Xcode/Android Studio, EAS Build 또는 `expo prebuild`)가 필요해서 이번 데모에는 포함하지 않았습니다. 지금 앱은 자체 실행형으로 카메라/갤러리에서 사진을 선택하는 방식입니다. 핵심 분석·저장 로직이 검증되면 다음 단계로 Share Extension을 얹는 걸 추천합니다.
