...
_PROMPT = """You are analyzing a photo that was pre-classified as "{category}" by a
first-pass vision model. Look at the image and extract structured data.
...
def analyze(image_bytes: bytes, category: Category, confidence: float, media_type: str = "image/jpeg") -> AnalyzeResponse:
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": _PROMPT.format(category=category)},
                ],
            }
        ],
    )
