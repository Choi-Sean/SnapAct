...
def classify_image(image_bytes: bytes, mock_category: str | None = None) -> tuple[Category, float]:
    """Returns (category, confidence). Falls back to a mock result when no
    Google credentials are configured, so the pipeline is testable end to end
    before real keys are provisioned."""
    if not settings.vision_enabled:
        category = mock_category if mock_category in VALID_CATEGORIES else "business_card"
        return category, 0.99  # type: ignore[return-value]
    ...
    text = (text_response.text_annotations[0].description.lower() if text_response.text_annotations else "")
    haystack = f"{labels} {text}"
