// ============================================================================
// LAYER 1 — on-device vision blocking gate (mobile/src/layer0/)
// ============================================================================
// The fail-closed policy layer for the Core ML L1 classifier. Mirrors the
// spike's L1Gate.swift exactly. The native module (modules/coreml-classify)
// only produces class probabilities; THIS decides BLOCK vs ROUTE.
//
// Runs BEFORE OCR and before any upload (see AnalyzeScreen.resolveAnalysis):
// a Tier 0 photo must never leave the device, and blocking must not depend on
// OCR succeeding (see .claude/rules/pipeline.md ordering).
import {
  classifyImage,
...
// than a low-confidence guess (pipeline.md: `unknown` is the primary path).
