...
make test           # 79개 테스트, 8개 스위트, 약 3.4초
```
...
| **OCR** | 한국어를 실제로 읽는다. `.fast` 가 한국어에서 선택되지 않는다. span 이 평탄화되지 않는다 |
| **OCR 중재** | 정정이 버튼을 **추가만** 한다. 모델이 없으면 사유를 남기고 라우팅 결과를 그대로 둔다 |
...
| OCR | ✅ 텍스트 span 까지. 필드 추출(L3)은 범위 밖 |
| 액션 버튼 순위 | ❌ 10단계 |
| 개인화 카운터 | ❌ 10단계 |
| 로그 기록 | ❌ 11단계 |
| 디버그 화면 내용 | ❌ 12단계 |
...
### Vision 이 실제로 지원하는 언어 (런타임 조회, 문서 아님)

| 인식 수준 | 언어 수 | 내용 |
|---|---|---|
| `.fast` | **6** | `en-US fr-FR it-IT de-DE es-ES pt-BR` — 전부 라틴 |
| `.accurate` | **30** | 위 6개 + `ko-KR` `ja-JP` `zh-Hans/Hant` `th-TH` `ar-SA` 등 |

**한국어·CJK 는 `.accurate` 전용입니다.** `ios-platform.md` 의 주장이 실측으로
확인됐습니다. 스크린샷이어도 한국어가 섞이면 `.fast` 로 내려갈 수 없고, 코드가
그걸 강제합니다 (안전장치를 제거하면 테스트가 실패합니다).

목록에 `vi-VT` 가 있습니다 — `vi-VN` 이 맞는 표기로 보이는데 Apple 이 그렇게
내놓습니다. 기억으로 설정을 쓰면 아무것도 매칭되지 않고 조용히 실패하는 사례라,
언어 매칭을 서브태그 접두사로 합니다.

### 비용

| | 시간 |
|---|---|
| 첫 호출 (파이프라인 초기화 포함) | **약 1,600 ms** |
| 이후 `.accurate` | 53 ms |
| 이후 `.fast` | 23 ms |

**첫 호출 1.6초가 중요합니다.** `pipeline.md` 의 "공유 → 액션 제안 3초 p90"
예산에서 절반을 먹습니다. 그래서 D-4 의 프로그레시브 방식(버튼 먼저, OCR 결과
나중)이 선택이 아니라 필수입니다.

한국어 읽기 확인: `영수증 합계 12,500원` → 정확히 읽힘.

## 4-5. ⚠️ Foundation Models — 이 기기에서는 꺼져 있습니다

```
FoundationModels: unavailable(appleIntelligenceNotEnabled)
```

**Apple Intelligence 가 활성화돼 있지 않아 모델 경로를 실기로 검증하지
못했습니다.** 지금 실제로 도는 것은 폴백 경로이고, 그쪽은 테스트로 덮여
있습니다 — 모델이 없으면 사유를 남기고 라우팅 결과를 그대로 둡니다.

켜서 확인하시려면: **시스템 설정 → Apple Intelligence & Siri** 에서 켜고
모델 다운로드가 끝난 뒤 `make test` 를 다시 돌리면 됩니다. 테스트가 가용
여부를 런타임에 보고 분기하므로, 켜져 있으면 **"가용한데 거절했다"** 를 실패로
잡습니다.

모델 없이도 검증되는 부분은 검증했습니다: `@Generable` 매크로가 실제로 닫힌
스키마(`first`/`second`/`cannotTell`)를 만들어냅니다. 이게 **모델이 두 후보
밖의 답이나 산문을 내놓을 수 없다는 보장**입니다.

### 정정은 추가만 합니다

`receipt ↔ bill_invoice` 는 실사진 262장에서 **37% 가 섞였습니다**(4-2절).
생김새가 같고 텍스트만이 구분합니다 — 과거 날짜면 영수증, 미래 기한 + 계좌번호면
고지서.

정정이 나와도 **기존 버튼을 지우지 않고 다른 후보를 함께 올립니다.** 사용자가
누르려던 버튼이 사라지는 것이 최악이기 때문입니다. 교체로 바꾸면 테스트가
실패합니다.

모델이 읽고도 못 가리면(`cannotTell`) 양쪽을 다 제시합니다 — 안 돌린 것과
읽고도 모르는 것은 다른 답이고, 후자에서 이미지의 추측만 남기면 그 정보를
버리는 셈입니다.
